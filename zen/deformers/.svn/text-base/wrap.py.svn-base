...
class Wrap(list):
	...
	def __init__(self,*args,**keywords):
		...
		for a in args:
			if\
			(
				(isinstance(a,basestring) and len(a.split('.'))>1 and str(mc.getAttr(a,type=True)) in ['mesh','nurbsCurve','nurbsSurface']) or
	...
	def create(self):
		...
		for wo in woHold:
			if\
			(
				(isinstance(self.wrapOptions[wo],basestring) and self.wrapOptions[wo]=='') or
